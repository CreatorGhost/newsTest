...
def configure_app():
    ...
    return embeddings, redis_instance
...
def create_app():
    app = Flask(__name__)
    check_file(path)
    embeddings, redis_instance = configure_app()
    data_store = load_data("cryptonews",embeddings)
    return app, path, embeddings, redis_instance ,data_store

app, path, embeddings, redis_instance ,data_store = create_app()
